...
console.log(numbers.forEach(el => console.log(el/2)))
...
console.log(cities.filter(el => el.at(-1) === "i"))
...
const detectedFruit = (fruit, index) => {
  switch(fruit.color){
    case "red": console.log(`The fruit with index ${index} is an apple.`)
                break
    case "orange": console.log(`The fruit with index ${index} is an orange.`)
                break
    default: console.log(`The fruit with index ${index} is neither apple or orange.`)
  }
}

fruits.forEach(fruit => detectedFruit(fruit, fruits.findIndex(el => el.color === fruit.color)))
...
const withRice = (total, food, callback) => {
  let currentTotal = total + food.price
  callback(`Total: ${currentTotal} Food: ${food.name} ${food.price}`)
  return currentTotal 
}

const printTotalAndFood = (food) => console.log(food)

console.log(foods.reduce((total, food) => food.tags.includes("rice") ? withRice(total, food, printTotalAndFood) : total, 0))
